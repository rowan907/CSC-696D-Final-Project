...
\begin{enumerate}
    ...
    \item At what were the development priorities for a repository at any given
        time.
    \item Can we identify a time when AI assisted development because popular on
        a given repository.
    \item What is the best visualization for giving developers an understanding of the
        evolution project structure over time.
\end{enumerate}
...
\begin{enumerate}
    \item A 'stabilized' Sediment Graph based on the work
        ...
        Bernhardsson proposes a unique way to underhand the age of the contents of a
    ...
    \item A commit graph and word cloud style view that allow users to select
        ...
        Git Metadata Analysis}. We seek to enhance there view by adding a time
    ...
    \item A grouping based visualization that clusters files or directories that
        ...
        to give users an understanding of how that changes overtime.
\end{enumerate}
...
\item
    ...
    This work shows an all-in-one visual analytic tool for examining git history
        ...
        technical aspects of creating visualization's with git meta-data. We
...
Git Repository Evolution} --- Git-Truck is a hierarchical file-organized-oriented 
    ...
    and the color of its nodes can represent a variety of interested factors 
    ...
    focus we have on in this project. Mainly, we emphasize the importance of seeing 
...
\item \href{https://arxiv.org/pdf/2508.12649}{ChangePrism:Visualizing the
    Essence of Code Changes} This paper gives another approach to visualizing
        code changes that relies on semantic Analysis. Though this papers focus
        is more on visualizing individual commits and discreet changes rather
        than the type of broad analysis of larger repositories we hope to do,
        but gives some good background on existing visual analytic techniques.
\end{itemize}
\subsection{Planned technical approach}
Our visualizations will use the `.git` files for the repos we are studying as a
...
aforementioned visualization techniques. We will the gather feedback from
professional software engineers on the usefulness of our visualization and
applicability of our tools to there current development workflow.
